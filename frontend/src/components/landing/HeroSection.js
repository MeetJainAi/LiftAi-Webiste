...
export default function HeroSection({ dashboardImage }) {
  ...
  return (
    ...
    >
      {/* Ambient glow - larger, more dramatic */}
      <div className="absolute top-1/4 right-[15%] w-[700px] h-[700px] bg-[#A3BCA7]/[0.07] rounded-full blur-[200px] pointer-events-none phone-glow" />
      <div className="absolute bottom-0 left-[20%] w-[400px] h-[400px] bg-[#A3BCA7]/[0.03] rounded-full blur-[150px] pointer-events-none" />

      <div className="max-w-[1200px] mx-auto px-6 md:px-12 w-full grid md:grid-cols-2 gap-12 md:gap-16 items-center">
        {/* Left: Text */}
        <div ref={textRef} className="space-y-8">
          {/* Badge — Von Restorff effect: isolate a unique element */}
          <div className="animate-hero-badge inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full border border-[#A3BCA7]/20 bg-[#A3BCA7]/[0.05]">
            <span className="w-1.5 h-1.5 rounded-full bg-[#A3BCA7] pulse-dot" />
            <span className="text-[#A3BCA7] text-[11px] tracking-[0.2em] uppercase font-medium">
              AI-Powered Training
            </span>
          </div>
          ...
          <h1 className="font-serif-heading text-5xl sm:text-6xl lg:text-7xl font-light leading-[1.05] tracking-tight animate-hero-text">
            Never wonder
            <br />
            what to <span className="gradient-text">train</span> again
          </h1>
          ...
          <div className="flex flex-wrap gap-4 pt-2 animate-hero-cta">
            <a
              ...
              className="group inline-flex items-center gap-2.5 bg-[#F0EDE8] text-[#0F0F0F] rounded-full px-8 py-3.5 text-sm font-medium
                         hover:bg-white transition-all duration-300 hover:-translate-y-[2px]
                         hover:shadow-[0_8px_30px_rgba(240,237,232,0.15)]"
            >
        ...
        {/* Right: Phone with dramatic glow */}
        <div ref={phoneRef} className="flex justify-center md:justify-end animate-hero-phone relative">
          {/* Glow ring behind phone */}
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="w-[280px] h-[280px] md:w-[340px] md:h-[340px] rounded-full bg-[#A3BCA7]/[0.08] blur-[80px] phone-glow" />
          </div>
      ...
      {/* Scroll indicator */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 scroll-indicator" data-testid="scroll-indicator">
        <ChevronDown className="w-5 h-5 text-[#9B9B9B]/40" />
      </div>
    </section>
  );
}
